...
def create_db_engine() -> Engine:
    ...
    safe_password = password.replace("*", "%2A") if password else ""
    return create_engine(
        f"mysql+pymysql://{user}:{safe_password}@{host}:{port}/{database}?charset=utf8mb4",
        pool_pre_ping=True,
    )
...
def inspect_table_csvs(parsed_output_dir: Path, pattern: str) -> tuple[list[str], int]:
    """Read only CSV headers for fast schema validation."""
    files = sorted(parsed_output_dir.glob(pattern))
    columns: list[str] = []
    seen = set()
    for path in files:
...
def align_to_existing_table(
    ...
    # Keep source file only for reporting; never load it into existing app tables.
...
def filter_append_new(
    ...
    if not usable_columns:
        ...
        if "match_id" in df.columns:
            existing_key_rows = get_existing_key_rows(engine, table_name, ["match_id"])
            if existing_key_rows.empty:
                return df
            existing_ids = set(pd.to_numeric(existing_key_rows["match_id"], errors="coerce").dropna().tolist())
            return df[~pd.to_numeric(df["match_id"], errors="coerce").isin(existing_ids)].copy()
        return df
...
def load_parsed_outputs_to_db(
    ...
    *,
    ...
    for spec in TABLE_SPECS.values():
        ...
        if mode == "append_new":
            df_to_load = filter_append_new(db_engine, table_name, df, key_columns)
        else:
            df_to_load = df
