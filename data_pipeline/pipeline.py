...
from .legacy import whoscored_parser, whoscored_scraper
from .load_db import (
    DbLoadMode,
    create_db_engine,
...
class PipelineRunner:
    ...
    def run(
        ...
        if mode in {"download", "full", "full-db"}:
            if self.resolve_incremental_state_mode(incremental_state) == "db":
                self.reset_temp_workspace()
            ...
            result.fixtures_rows = len(fixtures_df)
            should_download = not fixtures_df.empty
            if should_download:
                self.download_matches(fixtures_df)
            else:
                self.logger.info("No pending fixtures to download for source '%s'", self.source.name)

        if mode in {"parse", "full", "full-db"}:
            if should_download or self.has_pending_local_json():
                self.parse_matches()
            else:
                self.logger.info("No JSON files pending parse for source '%s'", self.source.name)

        if mode in {"validate-db"}:
            if self.has_pending_parsed_outputs():
                result.db_validation_ok = self.validate_db()
            else:
                self.logger.info("No parsed outputs found; skipping DB validation.")
                result.db_validation_ok = True
        ...
        if mode in {"load-db", "full-db"}:
            if self.has_pending_parsed_outputs():
                ...
                self.logger.info("No parsed outputs found; skipping DB load.")
                result.db_validation_ok = True
                result.db_loaded_rows = 0
            if cleanup_temp:
    ...
    def reset_temp_workspace(self) -> None:
        self.logger.info(
            "Resetting temporary workspace for DB-driven incremental run | matches_dir=%s | parsed_dir=%s",
            self.source.matches_dir,
            self.source.parsed_output_dir,
        )
        self.cleanup_temp_files()

    def has_pending_local_json(self) -> bool:
        return any(self.source.matches_dir.glob("*.json"))

    def has_pending_parsed_outputs(self) -> bool:
        return any(self.source.parsed_output_dir.glob("*.csv"))

    def _fetch_fixtures_for_source(self) -> pd.DataFrame:
        if self.source.family != "whoscored":
            raise ValueError(
                f"Unsupported source family '{self.source.family}' for the streamlined pipeline."
            )
