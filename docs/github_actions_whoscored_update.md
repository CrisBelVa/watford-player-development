...
## Requisitos técnicos ya cubiertos

Se ha ajustado:

- `scripts/run_pipeline_server.sh`
  - ahora admite `python`, `python3` o el binario indicado por `PYTHON_BIN`
- `prueba lucas/whoscored_scraping_fun.py`
  - ahora soporta modo headless para CI mediante `HEADLESS=1`
...
## Nota sobre el estado local del pipeline

El modo servidor actual limpia JSON y CSV temporales al final. Eso es correcto para automatización, pero hace que `data/pipeline/whoscored_championship/state.csv` no refleje bien todo lo que ya quedó cargado en base si se mira solo el disco local.

No bloquea la automatización, pero conviene tenerlo en cuenta al interpretar ese fichero.
